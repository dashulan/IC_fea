...
class LLL_Net(nn.Module):
    ...
    def __init__(self, model, remove_existing_head=False):
        ...
        # self.specHeads = nn.ModuleList()
        self.taskSpec = nn.ModuleList()
        self.task_cls = []
        self.task_offset = []
        self._initialize_weights()
        self.bnL = nn.BatchNorm1d(self.out_size)
    ...
    def add_head(self, num_outputs):
        ...
        # self.specHeads.append(nn.Linear(self.out_size, num_outputs,bias=False))
        # self.hiddens.append(nn.Linear(self.out_size,self.out_size,bias=False))
        self.taskSpec.append(nn.Sequential(nn.Linear(self.out_size,self.out_size),nn.BatchNorm1d(self.out_size),\
            nn.ReLU(inplace=True),nn.Linear(self.out_size,num_outputs,bias=False)))
    ...
    def hiddenForward(self,fea,t):
        y= self.taskSpec[t](fea)
        return y
