...
class Appr(Inc_Learning_Appr):
    ...
    def criterion(self, t, outputs, targets, outputs_old=None):
        ...
        for t_old in range(0,t):
            loss_dist+=self.cross_entropy(outputs[t_old],outputs_old[t_old],exp=1/self.T)
        # Current cross-entropy loss -- with exemplars use all heads
        # if len(self.exemplars_dataset) > 0:
        #     return loss_dist + torch.nn.functional.cross_entropy(torch.cat(outputs, dim=1), targets)
        return loss_dist + torch.nn.functional.cross_entropy(outputs[t], targets - self.model.task_offset[t])